...
\todo{Mention RANSAC threshold}
For each Planar Cell,
RANSAC-based plane model fitting is applied, getting a plane and separating the points into inliers and outliers.
...
various types of
point cloud sensors. Therefore, we test the performance of GroundSeg3D on datasets from 
challenging urban and natural environments with varied sensor configurations. For comparison, we selected state-of-the-art
ground segmentation algorithms mentioned in Table~\ref{tab:comparison_non_downsampled} and used their default parameters. 

We observed that some of the ground segmentation algorithms require 
...
\end{figure}
\fi
We further analyze the runtime of GroundSeg3D on various datasets (Table~\ref{tab:runtime}).
...
\begin{table}[tb]
  ...
  \hline
  \textbf{SemanticKITTI} & 86 \\ 
  \hline
  \textbf{UrbanNav} & 42 \\ 
  \hline
  \textbf{DOALS} & 60 \\ 
  \hline
  \textbf{BotanicGarden} & 21 \\
  \hline
  \textbf{RELLIS-3D} & 82 \\
  \hline
  \end{tabular}%
...
and False Negative [Purple] points are shown.
