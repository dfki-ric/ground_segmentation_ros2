...
For example, object and obstacle detection algorithms often detect ground points as false
    positives. Removal of ground points improves their performance and reduces computational burden \cite{Firkat2023}.
%
Additionally, ground points can be used for traversability analysis, navigation, and
    static map generation.
Detection of ground points is a necessary precursor step for extraction
...
solutions \cite{GPR,linefit,R-GPF,RANSAC,CascadedSeg,9466396,9981561}. Region growing and clustering is used to combine individual grid
...
Some algorithms are based on initial seed cells for the processing \cite{GPF}.
...
\begin{enumerate}
  ...
  \item \textbf{Proximity to Sensor:} Cells that are far 
...
%   coverage and connectivity of the region-growing process.
  ...
  and the back (3rd and 4th quadrants) in GroundSeg3D and use all four seed cells in all quadrants for GroundSeg3D$^{\star}$ as shown in Table~\ref{tab:comparison_non_downsampled}. The seed cells are then used for region growing.
  \end{enumerate}
...
For all experiments with GroundSeg3D, we use grids of sizes (2,2,10) and (2,2,0.5) in Phase-I and Phase-II respectively. The ground inlier threshold is 0.1 meters 
and the ground slope threshold is 30 degrees.

%combined table
\begin{table*}[tb]
  \centering
  \caption{Comparison of ground segmentation on SemanticKITTI sequences 00, 01, 04, and 09 with vegetation. As explained in section \ref{sec:region_growth}, two and four seed cells are used for expansion in GroundSeg3D and GroundSeg3D$^{\star}$ respectively.}.
  \label{tab:comparison_non_downsampled}
  \begin{tabular}{@{}l@{} l@{\ \ }l@{\ \ }l@{\ \ }l   l@{\ \ }l@{\ \ }l@{\ \ }l   rrrr}
  \toprule
   Algorithm   & \multicolumn{4}{c}{Average precision $\pm$ standard deviation (\%)} & \multicolumn{4}{c}{Average recall $\pm$ standard deviation (\%)} & \multicolumn{4}{c}{Average Time (ms)}  \\
...
\midrule
  ...
  GroundSeg3D^{\star}\quad{}  & $94.4\pm 5.0$ & $94.4\pm 4.5$ & $97.8\pm 1.6$ & $92.2\pm 5.2$ & $89.1\pm 6.3$ & $80.4\pm 6.8$ & $89.0\pm 2.6$ & $89.3\pm 4.3$ & 84& 80& 98 & \!\!102\\ 
  \bottomrule
\end{tabular}%
\end{table*}
...
\begin{figure}[tb]
  ...
  \caption{GroundSeg3D results on rough outdoor terrain at the Robotics Test Track at DFKI RIC}
  \label{fig:arter_all}
\end{figure}
...
We greatly acknowledge the open source Ground Segmentation Benchmark \cite{9466396,9981561} which we used for benchmarking.
%\footnote{https://github.com/url-kaist/Ground-Segmentation-Benchmark}
Parts of our algorithm were written with support of ChatGPT.
