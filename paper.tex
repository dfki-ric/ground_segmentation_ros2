...
While the threshold of 0.4 helps in distinguishing between Plane and Non-planar cells, it may lead to some false 
...
we selected some of the state-of-the-art ground segmentation works form literature. We used the standard paramters for the algorithms. Since some algorithms require 
...
and we did not update any parameters in our tests throughout the experimentation. The results of our GroundGrid3D are with the same set of paramters shown in table.
...
\subsection{GroundGrid3D Parameters}
...
In this section, we discuss the experimentation results. We take the SemanticKITTI sequence 0 to analyze the performance metrics with the state-of-the-art solutions.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[b]{0.5\textwidth}
      ...
      \label{fig:subfig4}
  \end{subfigure}
  \caption{Performance metrics of GroundGrid3D wrt Distance}
  \label{fig:main_fig}
\end{figure}
...
In the section, we compare the results of GroundGrid3D with other ground segmentation solutions on the SemanticKITTI dataset. The True Positive [yellow], False Positive [Blue], 
...
segmentation results of our GroundGrid3D at those frames.
...
\begin{figure*}
  ...
  \caption{BotanicGarden Dataset}
  \label{fig:botanicgarden_combined}
\end{figure*}

We get the second highest precison only second to the LineFit algorithm and fourth highest F1 score. The recall has a high standard deviation because of the inherent nature
of the region growth algorithm. The state-of-the-art solutions have a clear advantage in terms of runtime over GroundGrid3D. The precision are recall of GroundGrid3D are 
on par with the other solutions. Furthermore, since GroundGrid3D is not only developed for SemanticKITTI and does not require any parameter tuning, we feel that the 
...
\midrule
  ...
  \bottomrule
  GroundGrid3D & $94.79\pm 5.05$ & $86.92\pm 11.47$ & 90.69 & 85\\ 
  \bottomrule
\end{tabular}%
\end{table}
...
\midrule
  ...
  \bottomrule
  GroundGrid3D & $94.81\pm 4.35$ & $78.35\pm 14.79$ & 85.80 & 81\\ 
  \bottomrule
\end{tabular}%
\end{table}
...
\midrule
  ...
  \bottomrule
  GroundGrid3D & $97.46\pm 2.14$ & $86.65\pm 14.18$ & 91.74 & 98\\ 
  \bottomrule
\end{tabular}%
\end{table}
...
\midrule
  ...
  \bottomrule
  GroundGrid3D & $92.84\pm 4.79$ & $88.76\pm 5.71$ & 90.75 & 102\\ 
  \bottomrule
\end{tabular}%
\end{table}

We further analyze the runtime of GroundGrid3D on various datasets. This should give the end user a good idea of the expected runtime.
