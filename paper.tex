\documentclass[cocnference]{IEEEtran}
...
has been a standard sensor in autonomous systems \cite{}. The LIDAR uses bands of
...
\begin{itemize}
  \item Detection of ground points is a necessary precursor step for extraction
    ...
    robot navigation. [6,8] 
  \item Object and obstacle detection algorithms detect ground points as false
    positives. The detection can be improved when ground points aredetected and
...
approaches [X] which classifies the available state-of-the-art solutions.
...
individually. The grid cells may be purely 2D, 2.5D, or even 3D [X]. Some
approaches extend the grid cell and analyze the points in neighboring cells for
a better estimation of surface properties [X]. The modeling of the points in the
grid cells using  plane fitting and line extraction is a common approach in many
solutions [X]. Region growing and clustering is used to combine individual grid
cells into a larger region of connected cells [X]. Higher order inferences based
...
The development of PointNet [X] and its successor PointNet++ [X] give a unified
solution for various applications e.g. object and scene segmentation. Various
other solutions like GndNet [X], PointPiller [X], JCP [X], SectorGSnet [X]
...
Solution: We make use a phase-wise approach. In the first phase, we use a large
...
the junction of ground and non-ground points \paragraph*{Solution:} The grid
...
CellType &= 
  \begin{cases}
    \text{TentativeObstacle} & angle \ge 90° - Threshold \\
    \text{TentativeGround} & angle < 90° - Threshold
  \end{cases}
\end{align}
...
\subsection{KITTI [X]}
The KITTI dataset is a comprehensive collection of real-world data designed to
...
\end{tabular}
\todo[inline]{beautify table. Round values to reasonable precision}
\end{table}
