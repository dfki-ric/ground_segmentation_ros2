...
\author{\IEEEauthorblockN{Muhammad Haider Khan Lodhi\IEEEauthorrefmark{1} and Christoph Hertzberg\IEEEauthorrefmark{2}}
\IEEEauthorblockA{\textit{Robotics Innovation Center}\\
German Research Center for Artificial Intelligence (DFKI),
Bremen, Germany\\
Email: \IEEEauthorrefmark{1}mulo01@dfki.de, \IEEEauthorrefmark{2}christoph.hertzberg@dfki.de}%
}%

\maketitle

\todo{E-Mail format? I don't like my chhe01 address}
\begin{abstract}
...
\begin{figure*}[bt]
  ...
  \caption{GroundSeg3D: The original point cloud (represented as white points) is segmented into ground points (magenta) and non-ground points (blue). }
  \label{fig:results}
  \todo[inline]{Try forcing this to first page}
\end{figure*}
...
In this work, our primary focus was on extracting ground 
points with minimal parameter dependency, eliminating the need for parameter fine-tuning to ensure 
adaptability across various vehicles, robots, sensor configurations, for both indoor and outdoor environments. 
Our algorithm was tested using a consistent set of parameters on real-world datasets, featuring different 
...
individually. Such solutions are usually based occupancy grids or elevation maps \cite{5650541,9558794}. Some
...
PointNet++ \cite{qi2017pointnet++} give a unified
...
GroundSeg3D uses a one-shot processing approach for the point cloud data. The input to the algorithm is a single sample of point cloud and the output is 
ground and non-ground points. In the section, we explain the key processing steps performed on each input for robust ground segmentation.
...
The segmentation process is divided into two distinct phases to address different challenges in point segmentation. 
Each phase uses grid cells of varying heights to improve the accuracy of ground and non-ground detection, 
while also managing potential false segmentation.
...
\begin{figure}[H]
  ...
  input for Phase-II in which the coarse ground points are processed using grid cells with small height. The small height ensures only actual ground
  cells are expanded.}
  \label{fig:pipeline}
\end{figure}
...
This approach is designed to capture as many non-grounds structures as possible within each grid cell. However, this method comes with a trade-off. 
The increased cell height can lead to the false segmentation of ground points, 
...
The reduced cell height allows for more precise detection of ground and non-ground points.
...
classification is accurate, reflecting the true distribution of ground and non-ground points. Additionally, false positive ground points from Phase-I are
added back to the pool of non-ground points at the end of Phase-II.
...
\begin{table}[tbp]
  ...
  \begin{tabular}{@{}p{3.1cm}p{2cm}p{3cm}@{}}
\toprule
...
\begin{align}
cell_{\{x,y,z\}} &= point_{\{x,y,z\}} / cellsize_{\{x,y,z\}},
%cell_y &= point_y / cellsize_y \\
%cell_z &= point_z / cellsize_z
\end{align}
where $cellsize$ is a parameter. We explain our choice of parameters in section \ref{sec:parameters}.
...
\begin{figure*}
  ...
  dominating Eigen vectors (black) are shown. 
...
The Plane cells at this stage can not be considered as definitive ground because of the manner of points assigned to grid cells. It might be that the cell
...
If the ratio of the largest eigenvalue to the sum of all eigenvalues is smaller than 0.4, the cell is classified as non-planar. 
In this scenario, the cell does not exhibit two clearly dominant eigenvalues, 
suggesting that the points do not align well with a planar surface.

While the threshold of 0.4 helps in distinguishing between Planar and Non-planar cells, it may lead to some false 
classifications. Specifically, cells that should be classified as Plane or Non-planar might be incorrectly 
categorized due to the threshold. However, these misclassifications are addressed and corrected automatically 
during subsequent stages of the processing pipeline. Each Non-planar cell is individually processed in the segmentation stage.
...
When expanding the neighbors of a cell, we consider up to 24 neighboring cells in its vicinity. Empty cells are not stored in the 3D grid, 
which helps in focusing only on relevant neighboring cells. Therefore, to avoid including distant or irrelevant neighbors, we apply a maximum distance threshold.
...
\begin{table}[tbp]
  ...
  \label{tab:phase_behavior}
  \begin{tabular}{@{}p{1.8cm}p{1.0cm}p{1.2cm}p{0.5cm}p{1.1cm}p{1.cm}@{}}
  \toprule
  \textbf{Dataset} & \textbf{Environ-ment} & \textbf{Sensor} & \textbf{Year} & \textbf{Platform} & \textbf{Sensor Height}\\
  \midrule
  \textbf{SemanticKITTI \cite{Geiger2013IJRR}} & Urban & Velodyne HDL-64E & 2013 & Car & 1.723\,m \\ 
  \textbf{UrbanNav \cite{Hsunavi.602}} & Urban & Velodyne HDL-32E & 2021 & Car & 2.1\,m \\ 
  \textbf{DOALS \cite{9560730}} & Urban & Ouster OS0 128 & 2021 & Handheld & 2\,m \\ 
  \textbf{RELLIS-3D \cite{9561251}} & Natural & Ouster OS1 64 & 2022 & Mobile Robot & 1.16\,m \\
  \textbf{BotanicGarden \cite{10415477}} & Natural  & Velodyne VLP-16 & 2024 & Mobile Robot & 1.164\,m \\
  \bottomrule
  \end{tabular}%
\end{table}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/botanic_garden_all.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/doals_all.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/kitti_all.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/rellies3d_all.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/urban_nav_all.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/grouped_figures/ric.png}
  \caption{Ground segmentation results from data collected at DFKI RIC - MFH}
  \label{fig:ric}
\end{figure}

We further analyze the runtime of GroundSeg3D on various datasets. This should give the end user a good idea of the expected runtime.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/corrections/dual_phase_segmentation.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/dfki_track/actual_backyard.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/sloped terrain/arter_all.png}
  \caption{Ground segmentation results on rough outdoor terrain at the Robotics Test Track on DFKI RIC}
  \label{fig:arter_all}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/sloped terrain/kitti_all.png}
  \caption{Ground segmentation results on sloped outdoor terrain in SemanticKITTI}
  \label{fig:kitti_all}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/dfki_track/mfh.png}
  \caption{Ground segmentation results on various types of ground surfaces}
  \label{fig:kitti_all}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/corrections/neighborhood_correction.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/corrections/false_outliers/overview_1.png}
  \caption{Overview1}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/corrections/false_outliers/overview_2.png}
  \caption{Overview2}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/corrections/false_outliers/overview_3.png}
  \caption{Overview3}
...
\midrule
  ...
  CascadedSeg  & $91.65\pm 9$ & $71\pm10.4$ & 80 & 65.3 \\ 
...
\midrule
  ...
  LineFit      & $99\pm 1.52$ & $74.22\pm 9.46$ & 74.30 & 6 \\ 
...
\midrule
  ...
  R-GPF        & $84.56\pm 9$ & $93.43\pm 1.72$ & 88.77 & 18 \\ 
  GPR          & $94.98\pm2.00$ & $94.41\pm1.54$ & 94.19 & 13 \\ 
  CascadedSeg  & $98.20\pm 2.42$ & $72.50\pm3.72$ & 83.41 & 70 \\ 
  RANSAC       & $96\pm 2.04$ & $97\pm1.34$ & 96.15 & 42  \\ 
  LineFit      & $99.51\pm 0.51$ & $80.1\pm 4.64$ & 88.74 & 6.5 \\ 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/limitations/gaps.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/limitations/vegetation.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/limitations/combined.png}
